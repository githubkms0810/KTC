...
<div id="free_wrapper">
<form action="/admin/freelancer/<?=$mode?>" method="post"  onsubmit="" class="project_form floating-labels" enctype="multipart/form-data">
<fieldset>
	...
	<div>
		...
		<p class="project_select icon">
			<input value="<?=DEBUG === false ? my_set_value($row,"phone") : "개인번호3" ?>" placeholder="5678" class="budget" type="text" name="phone" id="free_phone" required>
		</p>
	</div>
	...
	<div class="icon">
		...
		<?php elseif($mode==="update"): ?>
			<input value="<?=$this->post_helper->extractUserNameOnEmail($row->email)?>" style="width:49.5%; display:inline-block;" class="email" type="text" name="email_first" id="free_email">
		<?php endif; ?>
	...
	<div>
		<div class="icon" style="width:79%; display:inline-block; margin-top:1px; margin-bottom:1px;">
			...
			<input value="<?=DEBUG === false ? my_set_value($row,"new_address") : "주소테스트" ?>" class="email" type="text" name="new_address" required id="sample4_roadAddress" readonly>
	...
	<div>
		...
		<p class="project_select icon_account">
			<select name="account_bank" class="budget" style="width:24%; display:inline-block;">
				...
				<option  <?=DEBUG === false ? set_select("account_bank","신한") : "selected"?>>신한</option>
			    <option  <?=set_select("account_bank","국민")?>>국민</option>
			</select>
